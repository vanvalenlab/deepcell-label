...
//   const args = {
//     parent: window.model.foreground,
...
//       storeDaughter: assign({ daughters: (context) => ({ ...context.daughters, [window.model.canvas.label]: 0 }) }),
...
//         return label !== 0 && label !== window.model.foreground && !(label in context.daughters)
//       }
//     }
//   }
// );

// export default deepcellLabelMachine;
...
const deepcellLabelMachine = Machine(
  {
    ...
    invoke: [
      ...
      },
      {
        ...
        src: toolMachine,
        // data: {
        //   foreground: (context) => context.foreground,
        //   background: (context) => context.background,
        // },
        // onDone: {
        //   actions: [
        //     assign({ tool: (event) => event.data.tool }),
        //     send((context, event) => ({ type: 'EDIT', args: event.data.args }), { to: 'api' }),
        //   ]
        // }
      },
      {
    ...
    states: {
      ...
      bubblingUp: {
        on: {
          BUBBLEUP:
            { actions: 'update', target: 'bubbledUp' },
